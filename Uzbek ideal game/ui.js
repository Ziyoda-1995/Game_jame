...
function selectLanguage(lang) {
    ...
    if (state.username) {
        ...
        document.getElementById('screen-intro').classList.add('active-screen');
    }
}
...
function openLanguageScreen() {
    ...
    langScreen.classList.add('active-screen');
}
...
function updateGlobalUI() {
    ...
    if (nameEl) {
        ...
        if (state.username && state.username.trim().length > 0) {
            ...
            nameEl.innerText = TRANSLATIONS[state.language].PLAYER;
        }
    ...
    if (teleBtn && state.hasTelescope) {
        ...
        teleBtn.innerText = TRANSLATIONS[state.language].ALREADY_BOUGHT;
    }

    // 4. Statik matnlarni yangilash
...
function updateStaticTranslations() {
    ...
    const mapping = {
        'lbl-login-title': t.LOGIN_TITLE,
        'lbl-login-btn': t.LOGIN_BTN,
        'lbl-map-title': t.MAP_TITLE,
        'lbl-profile-title': t.PROFILE_TITLE,
        'lbl-titles-header': t.TITLES,
        'lbl-shop-btn': t.SHOP_TITLE,
        'lbl-close-dash': t.CLOSE,
        'lbl-shop-header': t.SHOP_TITLE,
        'lbl-shop-balance': t.SHOP_BALANCE,
        'lbl-item-life-name': t.ITEM_LIFE_NAME,
        'lbl-item-life-desc': t.ITEM_LIFE_DESC,
        'lbl-item-hint-name': t.ITEM_HINT_NAME,
        'lbl-item-hint-desc': t.ITEM_HINT_DESC,
        'lbl-item-tele-name': t.ITEM_TELE_NAME,
        'lbl-item-tele-desc': t.ITEM_TELE_DESC,
        'lbl-shop-close': t.CLOSE,
        'lbl-exit-title': t.EXIT_TITLE,
        'lbl-exit-desc': t.EXIT_DESC,
        'lbl-yes': t.YES,
        'lbl-no': t.NO,
        'lbl-rating-title': t.RATING_TITLE,
        'lbl-close-rating': t.CLOSE,
        'lbl-win-fact': t.FUN_FACT,
        'lbl-next-level': t.NEXT_BTN,
        'lbl-quit-win': t.QQ_BTN,
        'lbl-back': t.BACK,
        'lbl-hud-level': t.LEVEL,
        'lbl-lang-btn': (state.language === 'uz' ? "🌐 Tillar" : "🌐 Языки"),
        'lbl-zoom': t.ITEM_TELE_NAME.split(' ')[1] + ' ' + (state.language === 'uz' ? 'KATTALASHTIRISH' : 'УВЕЛИЧЕНИЕ') // Dynamic zoom label if needed, or just simple text
    };

    // Fix for Zoom label which might not have a direct simple key
    const zoomEl = document.getElementById('lbl-zoom');
    if (zoomEl) zoomEl.innerText = (state.language === 'uz' ? "🔭 KATTALASHTIRISH" : "🔭 УВЕЛИЧЕНИЕ");
    ...
    // Class based updates
    document.querySelectorAll('.lbl-buy-btn').forEach(btn => btn.innerText = t.BUY);
    document.querySelectorAll('.lbl-coins').forEach(span => span.innerText = t.COINS);
}
...
function submitName() {
    ...
    if (!val) {
        showCustomAlert(TRANSLATIONS[state.language].PLEASE_ENTER_NAME, 'warning', TRANSLATIONS[state.language].WARNING);
        return;
    }
    state.username = val;
...
function renderTitlesGrid() {
    ...
    TITLES.forEach((title, idx) => {
        ...
        el.innerHTML = `
            ...
            <div class="title-item-name">${earned ? title.name[state.language] : (state.language === 'uz' ? 'Yopiq' : 'Закрыто')}</div>
        `;
        grid.appendChild(el);
    });
}
    ...
    switch (type) {
        case 'life':
            if (state.coins >= 50) {
                ...
                updateGlobalUI();
                showNotification(TRANSLATIONS[state.language].SHOP_BUY_LIFE_SUCCESS + state.lives, 'success');
            } else {
                showNotification(TRANSLATIONS[state.language].NOT_ENOUGH_COINS + '<br><br>Kerak: 50 💰<br>Mavjud: ' + state.coins + ' 💰', 'error');
            }
            break;
        ...
        case 'hint':
            if (state.coins >= 30) {
                if (!isGameActive) {
                    showNotification(TRANSLATIONS[state.language].SHOP_START_GAME_FIRST || 'Start Game First', 'warning');
                    return;
                }
                state.coins -= 30;
                localStorage.setItem('ul_coins', state.coins);
                updateGlobalUI();
                closeShop();
                showNotification(TRANSLATIONS[state.language].SHOP_BUY_HINT_SUCCESS, 'success');
                setTimeout(activateHint, 500);
            } else {
                showNotification(TRANSLATIONS[state.language].NOT_ENOUGH_COINS + '<br><br>Kerak: 30 💰<br>Mavjud: ' + state.coins + ' 💰', 'error');
            }
            break;

        case 'telescope':
            if (state.hasTelescope) {
                showNotification(TRANSLATIONS[state.language].SHOP_ALREADY_HAVE_TELESCOPE, 'info');
                return;
            }
            if (state.coins >= 100) {
                state.coins -= 100;
                ...
                showNotification(TRANSLATIONS[state.language].SHOP_BUY_TELESCOPE_SUCCESS, 'success');
            } else {
                showNotification(TRANSLATIONS[state.language].NOT_ENOUGH_COINS + '<br><br>Kerak: 100 💰<br>Mavjud: ' + state.coins + ' 💰', 'error');
            }
            break;
    }
...
function rate(n) {
    ...
    document.getElementById('rate-msg').innerText = (state.language === 'uz' ? "Baholandi: " : "Оценено: ") + n;
    state.rated = true;
...
function startTestMode(levelNum) {
    ...
    rangeData.forEach((data) => {
        ...
        if (isDesc) {
            questionText = (state.language === 'uz' ? "Ta'rifni toping: " : "Найдите описание: ") + `"${data.desc[state.language]}"`;
        } else {
            let fact = data.funFact[state.language];
            questionText = (state.language === 'uz' ? "Bu fakt qaysi yulduz turkumiga tegishli?\n" : "К какому созвездию относится этот факт?\n") + `"${fact}"`;
        }

        // Javob variantlarini tayyorlash
        const correctName = data.name[state.language];
        ...
        while (options.length < 4) {
            ...
            const randomName = randomItem.name[state.language];
...
function showTestQuestion() {
    ...
    document.getElementById('test-progress').innerText =
        (state.language === 'uz' ? "Savol " : "Вопрос ") + `${currentTest.currentQ + 1}/5`;
...
function finishTest() {
    ...
    if (currentTest.score === 5) {
        ...
        showCustomAlert(
            `Test yakunlandi!\n\n` +
            `To'g'ri javoblar: ${currentTest.score}/5\n\n` +
            (state.language === 'uz' ? "Unvon olish uchun barcha savollarga javob berishingiz kerak (5/5)" : "Нужно ответить на все вопросы для получения титула (5/5)"),
            'warning',
            TRANSLATIONS[state.language].TEST_FAILED || 'TEST NATIJASI'
        );
...
function showTitleEarned(titleData) {
    ...
    document.getElementById('earned-title-name').innerText = titleData.name[state.language];
    document.getElementById('earned-title-desc').innerText = titleData.desc[state.language];
...
function showCustomAlert(message, type = 'info', title = '') {
    ...
    let icon = type === 'success' ? '✅' : type === 'error' ? '❌' : type === 'warning' ? '⚠️' : 'ℹ️';
    let defaultTitle = type === 'success' ? 'MUVAFFAQIYAT!' : type === 'error' ? 'XATOLIK!' : type === 'warning' ? 'DIQQAT!' : "MA'LUMOT";
...
function showVictoryModal(levelIndex) {
    ...
    // O'ZBEK TILI MA'LUMOTLARINI OLISH (.uz)
    if (levelData) {
        if (nameEl) nameEl.innerText = levelData.name[state.language] || levelData.name.uz; // Fallback
        if (descEl) descEl.innerText = levelData.desc[state.language] || levelData.desc.uz; // Fallback
    }

    if (factEl) {
        if (zijData && zijData.funFact) {
            factEl.innerText = zijData.funFact[state.language] || zijData.funFact.uz;
        } else if (levelData && levelData.funFact) {
            factEl.innerText = levelData.funFact[state.language] || levelData.funFact.uz;
        } else {
            factEl.innerText = (state.language === 'uz' ? "Ulug'bek bu yulduz turkumini o'z rasadxonasida kuzatgan." : "Улугбек наблюдал это созвездие в своей обсерватории.");
        }
    }
...
function nextLevel() {
    ...
    if (nextIndex >= LEVELS.length) {
        showCustomAlert((state.language === 'uz' ? 'Tabriklaymiz! Barcha bosqichlar yakunlandi! 🎊' : 'Поздравляем! Все этапы завершены! 🎊'), 'success', 'A\'LO!');
